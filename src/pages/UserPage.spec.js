...
describe("UserPage", () => {
  describe("Layout", () => {
    ...
    it("displays the displayName@username when user data loaded", async () => {
      ...
      await waitFor(() => {
        text = queryByText("display1@user1");
    ...
    it("displays not found alert when user not found", async () => {
      ...
      await waitFor(() => {
        alert = queryByText("User not found");
  ...
  it("displays the edit button when loggedInUser matches to user in url", async () => {
    ...
    await waitFor(() => {
      editButton = queryByText("display1@user1");
  ...
  describe("ProfileCard Interactions", () => {
    ...
    it("returns to last displayName when display name is changed for another time but cancelled", async () => {
      const { queryByText, container } = await setupForEdit();
      ...
      const lastSaveData = container.querySelector("h4");
      expect(lastSaveData).toHaveTextContent("display1-update@user1");
    });

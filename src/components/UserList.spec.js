...
describe("UserList", () => {
  describe("Layout", () => {
    ...
    it("displays three items when listUser api returns three users", async () => {
      ...
      await waitFor(() => {
        userGroup = queryByTestId("usergroup");
        expect(userGroup.childElementCount).toBe(3);
      });
    });
    ...
    it("displays the displayName@username when listUser api returns three users", async () => {
      ...
      await waitFor(() => {
        firstUser = queryByText("display1@user1");
  ...
  describe("Interactions", () => {
    it("loads next page when clicked to next button", async () => {
      ...
      await waitFor(() => {
        secondPageUser = queryByText("display4@user4");
    ...
    it("loads previous page when clicked to previous button", async () => {
      ...
      await waitFor(() => {
        firstPageUser = queryByText("display1@user1");
    ...
    it("displays error message when loading other page fails", async () => {
      ...
      const previous = await waitFor(() => queryByText("< previous"));
      fireEvent.click(previous);

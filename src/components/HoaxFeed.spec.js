...
describe("HoaxFeed", () => {
  describe("Lifecycle", () => {
    ...
    it("calls loadNewHoaxCount with topHoax id and username when clicking Load More when rendered with user property", async () => {
      useFakeIntervals();
      apiCalls.loadHoaxes = jest
        .fn()
        .mockResolvedValue(mockSuccessGetHoaxesFirstOfMultiPage);
...
describe("Layout", () => {
  it("displays no hoax message when the response has empty page", async () => {
    ...
    await waitFor(() => {
      message = queryByText("There are no hoaxes");
      expect(message).toBeInTheDocument();
    });
  });
  it("displays not display no hoax message when the response has page of hoax", async () => {
    apiCalls.loadHoaxes = jest
      .fn()
  ...
  it("displays hoax content", async () => {
    ...
    const { queryByText } = setup();
    await waitFor(() => {
      hoaxContent = queryByText("This is the latest hoax");
...
describe("Interactions", () => {
  ...
  it("calls loadNewHoaxes with hoax id and username when clicking New Hoax Count Card", async () => {
    ...
    runTimer();
    await waitFor(() => {
      newHoaxCount = queryByText("There is 1 new hoax");
    });
    fireEvent.click(newHoaxCount);
